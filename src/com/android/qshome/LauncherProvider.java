...
public class LauncherProvider extends ContentProvider {
    private static final String TAG = "QsHome.LauncherProvider";
    private static final boolean LOGD = false;

    private static final String DATABASE_NAME = "launcher.db";
    
    //private static final int DATABASE_VERSION = 9;//8;
    private static final int DATABASE_VERSION = 10; // jz add support qs ext parameter

    public static final String AUTHORITY = "com.android.qshome.settings";
    
    public static final String TABLE_FAVORITES = "favorites";
    public static final String PARAMETER_NOTIFY = "notify";
    
    public static final String TABLE_APPS = "qsapplist";
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        private static final String TAG_FAVORITES = LauncherSettings.Favorites.TAG_FAVORITES;//"favorites";
        private static final String TAG_FAVORITE = LauncherSettings.Favorites.TAG_FAVORITE;//"favorite";
        private static final String TAG_CLOCK = "clock";
        private static final String TAG_SEARCH = "search";
        private static final String TAG_APPWIDGET = "appwidget";
        private static final String TAG_SHORTCUT = LauncherSettings.Favorites.TAG_SHORTCUT;//"shortcut";
        private static final String TAG_QS_FUNC = LauncherSettings.Favorites.TAG_QS_FUNC;//"qsfunc";
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("CREATE TABLE favorites (" +
                    ...
                    "uri TEXT," +
                    "displayMode INTEGER," +
                    "qsExtParam INTEGER NOT NULL DEFAULT 0" +
                    ");");
        ...
        private int copyFromCursor(SQLiteDatabase db, Cursor c) {
            ...
            while (c.moveToNext()) {
                ...
                values.put(LauncherSettings.Favorites.QS_EXT_PARAM, c.getInt(qsExtParamIndex));
                rows[i++] = values;
            }
        ...
        private int loadFavorites(SQLiteDatabase db) {
            ...
            int i = 0;
            try {
            	int nDefRes = R.xml.default_workspace;
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
                    ...
                    } else if (TAG_QS_FUNC.equals(name)) {
                        added = addQsFuncShortcut(db, values, a);
                    }
        ...
        private boolean addAppShortcut(SQLiteDatabase db, ContentValues values, TypedArray a,
            ...
            // jz
            final int qsExtParam = a.getInt(R.styleable.Favorite_qsExtParam, 0);
            try {
                ComponentName cn;
                try {
                ...
                if("com.android.contacts".equals(packageName) 
                	&& "com.android.contacts.DialtactsActivity".equals(className)){
                ...
                // jz
                final int container = a.getInt(R.styleable.Favorite_container, Favorites.CONTAINER_DESKTOP);
                
                if(container != Favorites.CONTAINER_DESKTOP){
	                values.put(Favorites.CONTAINER, container);
                }
                values.put(Favorites.QS_EXT_PARAM, qsExtParam);
        ...
        private boolean addAppWidget(SQLiteDatabase db, ContentValues values, ComponentName cn,
            ...
            try {
                ...
                values.put(Favorites.ITEM_TYPE, Favorites.ITEM_TYPE_APPWIDGET);
                if(Utilities.QS_ENABLE_WORKSPACE_5_5){
                	if(spanX == 4)
                		spanX = 5;
        ...
        private boolean addQsFuncShortcut(SQLiteDatabase db, ContentValues values,
                TypedArray a) {
            Resources r = mContext.getResources();

            final int funcKeyType = a.getInt(R.styleable.Favorite_qsFuncKey, 0);
            final int iconResId = a.getResourceId(R.styleable.Favorite_icon, 0);
            final int titleResId = a.getResourceId(R.styleable.Favorite_title, 0);
            // jz
            final int qsExtParam = a.getInt(R.styleable.Favorite_qsExtParam, Favorites.QS_EXT_PARAM_CUSTOM_APPS);
            
            if (funcKeyType < Favorites.ITEM_TYPE_QS_FUNC_START || qsExtParam <= 0 || (iconResId == 0 && titleResId == 0)) {
                Log.w(TAG, "Qs function key is missing title or icon resource ID");
                return false;
            }
            
//            int itemType = Favorites.getQsItemFuncTypeByKey(funcKey);
//            if(itemType < Favorites.ITEM_TYPE_QS_FUNC_START)
//            	return false;

            //values.put(Favorites.INTENT, funcKey);
            values.put(Favorites.TITLE, r.getString(titleResId));
            values.put(Favorites.ITEM_TYPE, funcKeyType);
            values.put(Favorites.SPANX, 1);
            values.put(Favorites.SPANY, 1);
            values.put(Favorites.ICON_TYPE, Favorites.ICON_TYPE_RESOURCE);
            values.put(Favorites.ICON_PACKAGE, mContext.getPackageName());
            ...
            if(qsExtParam > 0){
            	values.put(LauncherSettings.Favorites.CONTAINER,
                        LauncherSettings.Favorites.CONTAINER_HOTSET);
            }
            // jz
            values.put(Favorites.QS_EXT_PARAM, qsExtParam);
